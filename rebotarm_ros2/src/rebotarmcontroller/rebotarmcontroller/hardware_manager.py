...
class HardwareManager:
    ...
    def __init__(
        ...
        pose_compensation = hardware_data.get("pose_compensation", {}) or {}
        self.pose_compensation_enabled = bool(
            pose_compensation.get("enabled", False)
        )
        self.pose_z_offset = float(pose_compensation.get("z_offset", 0.0))
        self.min_user_z = float(
            pose_compensation.get("min_user_z", -float("inf"))
        )
        self.min_command_z = float(
            pose_compensation.get("min_command_z", -float("inf"))
        )
    ...
    def compensated_pose_z(self, z: float) -> float:
        """Apply calibrated Z offset before sending Cartesian targets to the SDK."""
        user_z = float(z)
        if user_z < self.min_user_z:
            raise RuntimeError(
                f"move_to_pose user z={user_z:.3f} below safe limit "
                f"{self.min_user_z:.3f}"
            )
        command_z = user_z
        if self.pose_compensation_enabled:
            # 用户输入按真实夹爪高度理解；SDK 目标点需要加上标定出来的 Z 偏移。
            command_z += self.pose_z_offset
        if command_z < self.min_command_z:
            raise RuntimeError(
                f"move_to_pose command z={command_z:.3f} below safe limit "
                f"{self.min_command_z:.3f}"
            )
        return command_z
