...
class JointStateMirror(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("source_gripper_state", "/rebotarm/gripper/state")
        ...
        self.declare_parameter("max_gripper_width", GRIPPER_MAX_WIDTH)
        self.declare_parameter("publish_rate", 20.0)   # 最大发布频率（Hz）
        self.declare_parameter("point_duration", 0.1)  # 每个轨迹点的时长（秒）
        self.declare_parameter("log_period", 3.0)      # 状态打印间隔（秒）
        ...
        self._latest_gripper_positions: list[float] | None = None
        self._latest_gripper_source = "joint_states"
        self._warned_missing: set[tuple[str, ...]] = set()
        self._published_once = False
        self._log_period = max(float(self.get_parameter("log_period").value), 0.0)
        ...
        self.create_subscription(
            JointMotorState,
            str(self.get_parameter("source_gripper_state").value),
    ...
    def _on_joint_state(self, msg: JointState) -> None:
        ...
        self._fill_gripper_from_fallback(positions)

        # 分别发布 arm 和 gripper 的轨迹命令
        arm_ok = self._publish_trajectory(self._arm_pub, self._arm_joints, positions)
        gripper_ok = self._publish_trajectory(
            self._gripper_pub, self._gripper_joints, positions
        )
        self._log_state(now, positions, arm_ok, gripper_ok)

    def _on_gripper_state(self, msg: JointMotorState) -> None:
        """把真机单电机夹爪位置换算成 Gazebo 左右夹爪 joint。

        正常情况下 /rebotarm/joint_states 已经包含 gripper_joint1/2。
        这里作为兜底，避免某些驱动配置只发布 /gripper/state。
        """
        joint_pos = self._hardware_gripper_to_sim(float(msg.position))
        self._latest_gripper_positions = [joint_pos for _ in self._gripper_joints]
        self._latest_gripper_source = "gripper/state"

    def _fill_gripper_from_fallback(self, positions: dict[str, float]) -> None:
        if all(name in positions for name in self._gripper_joints):
            self._latest_gripper_source = "joint_states"
            return
        if self._latest_gripper_positions is None:
            return
        for name, value in zip(self._gripper_joints, self._latest_gripper_positions):
            positions.setdefault(name, value)

    def _hardware_gripper_to_sim(self, position: float) -> float:
        ...
        ratio = 0.0 if span == 0.0 else (position - closed_position) / span
        ratio = max(0.0, min(1.0, ratio))
        return ratio * max_width * 0.5
    ...
    def _publish_trajectory(
        ...
        if missing:
            ...
            if key not in self._warned_missing:
                self.get_logger().warn(
                    f"skip mirror command, missing joints: {', '.join(missing)}"
                )
        ...
        # 保持 stamp=0，表示控制器收到后立即执行。
        # 不用本节点 wall time，避免和 Gazebo /clock 时间基准不一致。
        ...
        if not self._published_once:
            self.get_logger().info(
                "mirroring hardware joint_states to Gazebo controllers"
            )
    ...
    def _log_state(
        ...
        arm_text = ", ".join(
            f"{name}={value:.3f}" if value is not None else f"{name}=missing"
            for name, value in zip(self._arm_joints, arm_values)
        )
        gripper_text = ", ".join(
            f"{name}={value:.4f}" if value is not None else f"{name}=missing"
            for name, value in zip(self._gripper_joints, gripper_values)
        )
        available_gripper = [v for v in gripper_values if v is not None]
        width = 2.0 * sum(available_gripper) / len(available_gripper) if available_gripper else None
        width_text = f"{width:.4f} m" if width is not None else "unknown"
        self.get_logger().info(
            "mirror state | "
            f"arm_ok={arm_ok} gripper_ok={gripper_ok} | "
            f"arm: [{arm_text}] | "
            f"gripper: [{gripper_text}] width={width_text} "
            f"source={self._latest_gripper_source}"
        )
