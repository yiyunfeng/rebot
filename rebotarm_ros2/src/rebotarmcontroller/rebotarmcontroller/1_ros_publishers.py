"""
ros_publishers 模块 — 关节状态与机械臂状态发布器
================================================

本模块负责将硬件层读取的关节/夹爪状态，按 ROS2 标准消息格式定时发布。

**发布的话题**：
  - /{namespace}/joint_states       → sensor_msgs/JointState（标准关节状态）
  - /{namespace}/joints/{name}/state → rebotarm_msgs/JointMotorState（单关节详情）
  - /{namespace}/gripper/state       → rebotarm_msgs/JointMotorState（夹爪状态）
  - /{namespace}/arm_status          → rebotarm_msgs/ArmStatus（机械臂整体状态，latched）
"""

from __future__ import annotations

from rclpy.qos import DurabilityPolicy, QoSProfile, ReliabilityPolicy
from rebotarm_msgs.msg import ArmStatus, JointMotorState  # 自定义消息：机械臂总状态 / 单电机状态
from sensor_msgs.msg import JointState                    # ROS 标准 JointState 消息

# 夹爪最大开口宽度（米），用于将电机位置线性映射到关节角度
...
def _gripper_motor_to_joint_position(
    ...
    映射逻辑：
      - close_position → 0（闭合）
      - open_position → GRIPPER_MAX_WIDTH * 0.5（最大半开，单指行程）
      - 中间位置按比例线性插值，并 clamp 到 [0, 半开宽度]

    Args:
        position:       当前电机位置（原始单位）
        open_position:  夹爪全开时的电机位置
        close_position: 夹爪闭合时的电机位置

    Returns:
        映射后的关节位置（米），范围 [0, 0.045]
    """
...
class JointStatePublisher:
    """
    关节状态发布器 —— 定时从 HardwareManager 读取状态并发布到 ROS2 话题。

    设计要点：
      - 使用定时器（create_timer）按固定频率发布，而非事件驱动
      - 单关节状态使用独立话题发布（每个关节一个 publisher）
      - arm_status 使用 latched QoS（TRANSIENT_LOCAL），新订阅者能立即收到最新状态
      - 夹爪状态单独发布，并同步合并到 JointState 消息中
    """
    ...
    def __init__(self, node, hardware, namespace: str, rate_hz: float) -> None:
        ...
        self._publisher = node.create_publisher(
            ...
            node.sensor_qos, callback_group=node.reentrant_group,
        )

        # ----- 单关节状态发布器（字典）-----
        self._joint_state_publishers = {
            name: node.create_publisher(
                JointMotorState, f"/{namespace}/joints/{name}/state",
                node.sensor_qos, callback_group=node.reentrant_group,
            )
            for name in hardware.joint_names
        }

        # ----- 机械臂整体状态发布器（latched 模式）-----
        ...
        self._status_publisher = node.create_publisher(
            ArmStatus, f"/{namespace}/arm_status",
            latched_qos, callback_group=node.reentrant_group,
        )
        ...
        if hardware.has_gripper:
            self._gripper_state_publisher = node.create_publisher(
                JointMotorState, f"/{namespace}/gripper/state",
                node.sensor_qos, callback_group=node.reentrant_group,
            )
        ...
        self._timer = node.create_timer(
            period, self.publish, callback_group=node.reentrant_group,
        )
        self.publish_status()

    def publish(self) -> None:
        """定时回调：从硬件读取最新状态并发布所有话题。"""
        try:
    ...
    def publish_status(self, *, read_hardware: bool = True) -> None:
        """发布机械臂整体状态（ArmStatus 消息，latched QoS）。"""
        msg = ArmStatus()
        msg.header.stamp = self._node.get_clock().now().to_msg()
