               ...
               float minAngleDiff, float lengthRatio)
{
  ...
  for (it1 = forces.begin(); it1 != forces.end(); ++it1)
  {
    ...
    for (it2 = it1 + 1; it2 != forces.end(); ++it2)
    {
      ...
      RCLCPP_DEBUG_STREAM(rclcpp::get_logger("grasp"), "Angular distance is " << angle << " while the min is " << minAngleDiff/2);
      ...
      if (angle > minAngleDiff/2)
      {
        ...
        RCLCPP_DEBUG_STREAM(rclcpp::get_logger("grasp"), "Ratio is " << ratio << " while the min is " << lengthRatio);
