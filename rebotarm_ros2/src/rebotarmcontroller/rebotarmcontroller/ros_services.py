...
class ArmServices:
    ...
    def move_to_pose_ik(self, request, response):
        try:
            ...
            command_z = self._hardware.compensated_pose_z(z)
            if command_z != z:
                self._node.get_logger().info(
                    "move_to_pose_ik z compensation: "
                    f"user_z={z:.3f}, offset={self._hardware.pose_z_offset:.3f}, "
                    f"command_z={command_z:.3f}"
                )
            ok, q_solution = self._hardware.move_to_pose_ik(
                x, y, command_z, roll, pitch, yaw
            )
            response.success = ok
