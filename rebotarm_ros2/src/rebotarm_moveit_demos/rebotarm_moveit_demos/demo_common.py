...
class MoveItDemoBase:
    ...
    def wait_for_execute_server(self) -> bool:
        ...
        self.node.get_logger().error(
            "MoveIt action /execute_trajectory is not available"
        )
        return False
    ...
    def _joint_state(
        self, joint_values: list[float], is_diff: bool = True
    ) -> RobotState:
        return RobotState(
            is_diff=is_diff,
            joint_state=JointState(
                name=list(self.joint_names), position=list(joint_values)
            ),
        )
    ...
    def compute_ik_joint_target(
        ...
        if response is None or response.error_code.val != MoveItErrorCodes.SUCCESS:
            ...
            log(f"Failed to compute {label}: code={code}")
            return None
    ...
    def execute_trajectory(
        self, trajectory: RobotTrajectory, timeout_sec: float
    ) -> bool:
